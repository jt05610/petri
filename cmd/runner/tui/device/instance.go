...
func makeItems(instances []*device.Instance) []list.Item {
...
func InstanceModel(instances []*device.Instance) *InstModel {
	delegateKeys := newDelegateKeyMap()
	listKeys := newListKeyMap()

	// Setup list
	delegate := newItemDelegate(delegateKeys)
	groceryList := list.New(makeItems(instances), delegate, 0, 0)
